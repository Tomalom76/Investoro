...
import logging
import sys
import mlflow # Pozostawiam na wypadek przyszłego użycia, ale nie jest aktywnie używane
...
MLFLOW_TRACKING_URI = "http://127.0.0.1:5000" # Pozostaje dla spójności z watcherem
LOGGED_MODEL_URI_FROM_MLFLOW = 'runs:/daa9b673ba654fccbfdffb6f05a6ef75/model' # Zaktualizowane
MODEL_OUTPUT_DIR_FROM_WATCHER = os.path.join(BASE_DIR, "DATA_OUT") # Gdzie watcher zapisuje model
MODEL_FILENAME_FROM_WATCHER = "0_best_price_modelLGBM_auto" # Nazwa pliku bez .pkl
...
EXPECTED_INPUT_SCHEMA = {
    ...
    'Archive': str,
    'VoivodeshipNumber': pd.Float64Dtype(),
...
def load_local_pycaret_model_once():
    ...
    if loaded_model_global is None:
        ...
        try:
            ...
            loaded_model_global = pycaret_load_model(model_path, verbose=False) # verbose=False, aby nie było tabeli z setupu
...
def process_json_file(json_file_path):
    ...
    if not input_json_data_list:
        logging.warning(f"Plik JSON {json_file_path} nie zawiera danych.")
        return
    ...
    try:
        ...
        predictions_df = pycaret_predict_model(model_pipeline, data=df_for_model_input) # Używamy funkcji z PyCaret
        
        output_json_data_list = []
        raw_predictions_for_log = [] # Do logowania surowych predykcji
        ...
        for i, original_record_dict in enumerate(input_json_data_list):
            ...
            raw_predictions_for_log.append(predicted_value_raw) # Loguj surową wartość
            ...
            if price_key_original_case:
                ...
                new_record_output['PredictedPriceByModel'] = formatted_prediction
            output_json_data_list.append(new_record_output)
        
        # Logowanie przewidzianych wartości
        if raw_predictions_for_log:
            if len(raw_predictions_for_log) == 1:
                logging.info(f"Przewidziana cena dla pliku {os.path.basename(json_file_path)}: {formatted_prediction} (surowa: {raw_predictions_for_log[0]})")
            else:
                logging.info(f"Przewidziane ceny dla pliku {os.path.basename(json_file_path)} (pierwsza, jeśli wiele): {output_json_data_list[0].get('PredictedPriceByModel', 'Brak')} (surowa: {raw_predictions_for_log[0]})")


        if isinstance(input_json_data_raw, dict) and len(input_json_data_raw) >= 1:
            first_key = list(input_json_data_raw.keys())[0]
            if isinstance(input_json_data_raw[first_key], dict):
